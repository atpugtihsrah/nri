This document describes how to set up a test environment to test the NRI Identity Plugin
...
**Why:** The kustomize overlay in `contrib/kustomize/identity-injector/` deploys the NRI Identity Plugin as a DaemonSet in the `kube-system` namespace. The DaemonSet mounts the NRI socket, the SPIRE admin socket, and the host directory used for SVID artifacts.
